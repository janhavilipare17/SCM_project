    ...
    <div style={styles.dashboard}>
      ...
      <aside style={styles.sidebar}>
        ...
        <nav style={styles.nav}>
          {["shipments", "inventory", "track", "analytics", "settings"].map((tab) => (
            <button
              key={tab}
              style={{
                ...styles.navItem,
      ...
      <main style={styles.main}>
          ...
          <div>
            ...
            <div style={styles.section}>
              ...
              <div style={styles.rightBox}>
                ...
                <input style={styles.input} placeholder="Shipment ID" value={newShipment.id} onChange={(e) => setNewShipment({ ...newShipment, id: e.target.value })} />
                <input style={styles.input} placeholder="From (Manufacturer)" value={newShipment.from} onChange={(e) => setNewShipment({ ...newShipment, from: e.target.value })} />
                <input style={styles.input} placeholder="To (Retailer)" value={newShipment.to} onChange={(e) => setNewShipment({ ...newShipment, to: e.target.value })} />
                <input style={styles.input} type="date" value={newShipment.date} onChange={(e) => setNewShipment({ ...newShipment, date: e.target.value })} />
                <select style={styles.input} value={newShipment.status} onChange={(e) => setNewShipment({ ...newShipment, status: e.target.value })}>
                  <option>Pending</option>
                  <option>In Transit</option>
                  <option>Delivered</option>
                  <option>Returned</option>
                </select>
                <button style={styles.addBtn} onClick={handleAddShipment}>➕ Add Shipment</button>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "track" && (
          <div style={styles.trackingSection}>
            <h3>Track Orders</h3>
            <p>Monitor shipment status and delivery routes.</p>
            <div style={styles.trackingBox}>
              <p><b>Feature:</b> Real-time tracking via QR/Blockchain ID</p>
              <p><b>Status:</b> Integration coming soon!</p>
            </div>
          </div>
        )}
        ...
        {activeTab === "settings" && (
          <div>
            <h3> Settings</h3>
            <p>Manage your distributor profile and preferences.</p>
            <div style={styles.rightBox}>
              <input style={styles.input} placeholder="Distributor Name" />
